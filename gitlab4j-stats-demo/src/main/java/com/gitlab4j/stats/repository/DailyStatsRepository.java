...
public interface DailyStatsRepository extends JpaRepository<DailyStats, Long> {
    ...
    @Query("SELECT d FROM DailyStats d WHERE d.statDate BETWEEN :startDate AND :endDate ORDER BY d.statDate DESC, d.linesChanged DESC")
    List<DailyStats> findByStatDateBetweenOrderByStatDateDescLinesChangedDesc(
            @Param("startDate") LocalDate startDate, 
            @Param("endDate") LocalDate endDate);

    @Query("SELECT d FROM DailyStats d WHERE d.developerEmail = :email AND d.statDate BETWEEN :startDate AND :endDate ORDER BY d.statDate DESC")
